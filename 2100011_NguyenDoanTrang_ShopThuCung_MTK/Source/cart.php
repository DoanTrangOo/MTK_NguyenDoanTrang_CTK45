...
<body>
	...
	<section class="shopping-cart">
		...
		<h1 class="title">Các sản phẩm đã thêm</h1>
   ...
   ?>
			<form action="" method="POST" class="box">
				<a href="cart.php?delete=<?= $fetch_cart['id']; ?>" class="fas fa-times"
					onclick="return confirm('Chắc chưa?');"></a>
				...
				<div class="flex-btn">
					...
					<input type="submit" value="Sửa" name="update_qty" class="option-btn">
				</div>
				<div class="sub-total"> Tạm:
					<span><?= number_format($sub_total = ($fetch_cart['price'] * $fetch_cart['quantity']), 0, ',', ','); ?>
						VNĐ</span>
				</div>
			</form>
			<?php
      ...
      echo '<p class="empty">Trong giỏ chưa có sản phẩm nào</p>';
   }
   ?>
		</div>
		...
		<div class="cart-total">
			...
			<a href="cart.php?delete_all" class="delete-btn <?= ($grand_total > 1)?'':'disabled'; ?>">Xoá hết</a>
